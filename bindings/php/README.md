...
Биндинги **генерируются** из Go-исходника, поэтому его поверхность не может от них
убежать незаметно: см. [Синхронизация](#синхронизация).
...
1. **Кодогенерация.** Поверхность не пишется руками, поэтому расходиться нечему:
   ...
   Отслеживается: какие символы связаны и почему остальные нет, их сигнатуры, их
   doc-комментарии и форма каждой возвращаемой структуры — вплоть до имён полей,
   их типов и `json`-тегов. Тела методов не отслеживаются, и это правильно:
   переписать реализацию связанного метода, не трогая сигнатуру, — значит оставить
   биндинги верными.

   Если Go-имя параметра нельзя перенести (совпадает с C-параметром хендла,
   не может быть PHP-переменной вроде `$this`), генератор падает с кодом 1 и
   называет метод и имя, а не выпускает не собирающийся артефакт.

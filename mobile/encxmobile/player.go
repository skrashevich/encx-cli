...
func (c *EncClient) SendCode(gameID, levelID, levelNumber int64, code string) (string, error) {
	model, err := c.client.SendCode(c.bg(), int(gameID), int(levelID), int(levelNumber), code)
	if err != nil {
		return "", err
	}
	return marshalJSON(model)
...
func (c *EncClient) SendBonusCode(gameID, levelID, levelNumber int64, code string) (string, error) {
	model, err := c.client.SendBonusCode(c.bg(), int(gameID), int(levelID), int(levelNumber), code)
	if err != nil {
		return "", err
	}
	return marshalJSON(model)
